...
from src.providers.metalgrupsa.catalog import classify_document_kind
...
def resolve_reference(reference: str, name: str, catalog_rows: list[dict]) -> dict:
    ...
    if not catalog_rows:
        return _build_not_found(reference, name, "metalgrupsa_catalog_empty")
    ...
    if best_row is None or best_score < 260:
        return _build_not_found(reference, name, "metalgrupsa_no_catalog_match")
    ...
    return {
        ...
        "fallback_title": "",
        "fallback_pdf_url": "",
        "notes": "metalgrupsa_catalog_match",
    }
