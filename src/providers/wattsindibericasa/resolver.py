...
from src.providers.wattsindibericasa.catalog import classify_document_kind
...
def resolve_reference(reference: str, name: str, catalog_rows: list[dict]) -> dict:
    ...
    if not catalog_rows:
        return _build_not_found(reference, name, "wattsindibericasa_catalog_empty")
    ...
    if best_row is None or best_score < 260:
        return _build_not_found(reference, name, "wattsindibericasa_no_catalog_match")
    ...
    if len(exact_ref_matches) > 1 and not _has_strong_name_match(name, matched_name):
        return _build_not_found(reference, name, "wattsindibericasa_ambiguous_reference_requires_name")
    ...
    return {
        ...
        "fallback_title": "",
        "fallback_pdf_url": "",
        "notes": "wattsindibericasa_catalog_match",
    }
