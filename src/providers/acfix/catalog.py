...
def classify_document_kind(row: dict) -> str:
    ...
    if doc_type in {"ficha_tecnica", "datasheet", "technical_sheet"}:
...
def _normalize_catalog_row(row: dict) -> dict:
    ...
    normalized_row["brand"] = clean_spaces(normalized_row.get("brand", "")) or "AC-FIX"
    ...
    normalized_row["catalog_page"] = clean_spaces(normalized_row.get("catalog_page", ""))
    normalized_row["search_text"] = clean_spaces(normalized_row.get("search_text", ""))
    normalized_row["catalog_notes"] = clean_spaces(normalized_row.get("catalog_notes", ""))
