...
def classify_document_kind(row: dict) -> str:
    ...
    if doc_type in {"ficha_tecnica", "datasheet", "data_booklet", "catalogo_tecnico", "fichaproducto_pdf"}:
...
def _normalize_catalog_row(row: dict) -> dict:
    ...
    normalized_row["brand"] = clean_spaces(normalized_row.get("brand", "")) or "nordair"
